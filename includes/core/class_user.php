...
class User {
    ...
    public static function users_list($d = []) {
        ...
        // info
        $q = DB::query("SELECT users.user_id, users.plot_id, users.first_name, users.last_name, users.phone, users.email, users.last_login, plots.status, plots.billing, plots.number, plots.size, plots.price, plots.base_fixed, plots.electricity_t1, electricity_t2, plots.updated
            FROM users 
            LEFT JOIN plots ON users.plot_id = plots.plot_id
            ".$where." ORDER BY user_id LIMIT ".$offset.", ".$limit.";") or die (DB::error());
        while ($row = DB::fetch_row($q)) {
            $items[] = [
                'id' => (int) $row['user_id'],
                'first_name' => $row['first_name'],
    ...
    public static function user_edit_update($d = []) {
        ...
        $sql = "INSERT INTO users(first_name, last_name, phone, email, plot_id) VALUES(
              '$first_name', 
              '$last_name', 
              '$phone', 
              '$email',
